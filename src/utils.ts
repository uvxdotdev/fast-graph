 ...
 */
export function addRandomVelocity(node: GraphNode, minSpeed: number = 0.01, maxSpeed: number = 0.05): GraphNode {
 ...
 */
export function updateNodePosition(node: GraphNode, deltaTime: number): GraphNode {
  if (!node.vx && !node.vy) {
    return node;
  }
 ...
 */
export function applyDamping(node: GraphNode, dampingFactor: number = 0.99): GraphNode {
  if (!node.vx && !node.vy) {
    return node;
  }
 ...
 */
export function applySpringForces(
  nodes: GraphNode[], 
  edges: GraphEdge[], 
  restLength: number = 0.1, 
  springConstant: number = 0.01,
