...
const loadWasmModule = async (): Promise<any> => {
    ...
    wasmModulePromise = (async () => {
        try {
            ...
            throw new Error(
                `Failed to load FastGraph WASM module: ${error instanceof Error ? error.message : String(error)}`,
            );
        }
    })();
...
export const FastGraph: React.FC<FastGraphProps> = ({
    ...
    const componentId = useRef(
        `fast-graph-${Math.random().toString(36).substr(2, 9)}-${Date.now()}`,
    );
    ...
    const screenToWorld = useCallback(
        (screenX: number, screenY: number) => {
            ...
            const originalPixelX =
                canvasX / cameraRef.current.zoom + cameraRef.current.x;
            const originalPixelY =
                canvasY / cameraRef.current.zoom + cameraRef.current.y;
    ...
    useEffect(() => {
        if (animatedNodes.length > 0) {
            console.log(
                ...
                animatedNodes.slice(0, 3).map((node, i) => ({
                    index: i,
    ...
    useEffect(() => {
        ...
        const animate = (currentTime: number) => {
            ...
            setAnimatedNodes((prevNodes) => {
                const hasVelocity = prevNodes.some(
                    (node) => (node.vx && node.vx !== 0) || (node.vy && node.vy !== 0),
                );
                    ...
                    try {
                        ...
                        if (
                            ...
                            rendererRef.current.integrate_physics(
                                ...
                                springConstant,
                                restLength,
                                0.05, // repulsion_strength
                                0.5, // repulsion_radius
                            );
                            return prevNodes; // Skip CPU physics if GPU succeeded
                        }
                    } catch (err) {
                        console.warn(
                            "GPU acceleration failed, falling back to CPU physics:",
                            err,
                        );
                    }
                }
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (nodes.length > maxNodes) {
                console.warn(
                    `FastGraph: Node count (${nodes.length}) exceeds maximum (${maxNodes}). Only first ${maxNodes} nodes will be rendered.`,
                );
            }

            if (edges.length > maxEdges) {
                console.warn(
                    `FastGraph: Edge count (${edges.length}) exceeds maximum (${maxEdges}). Only first ${maxEdges} edges will be rendered.`,
                );
            }
        } catch (err) {
            ...
            try {
                ...
                for (const edge of edges) {
                    ...
                    if (sourceNode && targetNode) {
                        const sourcePos = normalizedToCanvas(
                            sourceNode.x,
                            sourceNode.y,
                            canvas.width,
                            canvas.height,
                        );
                        const targetPos = normalizedToCanvas(
                            targetNode.x,
                            targetNode.y,
                            canvas.width,
                            canvas.height,
    ...
    const handleResize = useCallback(() => {
        ...
        const targetWidth = Math.min(
            Math.floor(containerWidth * pixelRatio),
            maxDimension,
        );
        const targetHeight = Math.min(
            Math.floor(containerHeight * pixelRatio),
            maxDimension,
        );
    ...
    useEffect(() => {
        ...
        const init = async () => {
            try {
                ...
                const maxDimension = 2048;
                const targetWidth = Math.min(
                    Math.floor((rect.width || width) * pixelRatio),
                    maxDimension,
                );
                const targetHeight = Math.min(
                    Math.floor((rect.height || height) * pixelRatio),
                    maxDimension,
                );
            ...
            {isGraphMode && isInitialized && !error && (
                ...
                >
                    ...
                    <div style={{ marginBottom: "4px" }}>
                        <span style={{ color: "#FFC107" }}>Hold Shift</span> to enable
                        panning
                    </div>
                    <div style={{ marginBottom: "4px" }}>
                        <span style={{ color: "#2196F3" }}>PAN button</span> toggles manual
                        mode
                    </div>
                    <div style={{ marginBottom: "4px" }}>
                        <span style={{ color: "#4CAF50" }}>Drag nodes</span> when not
                        panning
                    </div>
                    <div style={{ marginBottom: "4px" }}>
                        <span style={{ color: "#FF9800" }}>Focus canvas</span> = no page
                        scroll
                    </div>
